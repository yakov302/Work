...
{
	if(capacity == 0 || hash_function == NULL || equality_function == NULL)
		return NULL;
	...
	hash_map->m_equality_function = equality_function;
...
Map_return hash_map_insert(HashMap* map, const void* key, const void* value)
{
    ...
    {
        if(list_is_exists(map->m_lists[index], map->m_equality_function, key) == YES)
...
Map_return hash_map_remove_and_free(HashMap* map, const void* key)
{
	...
	ListItr it = find_first(map->m_lists[index], map->m_equality_function, key);
...
{
	if(map == NULL || key == NULL || key_ptr == NULL || *key_ptr == NULL || value_ptr == NULL || *value_ptr == NULL)
...
int hash_map_is_exists(const HashMap* map, const void* key)
{
	...
	return list_is_exists(map->m_lists[index], map->m_equality_function, key);
}

Map_return hash_map_find(const HashMap* map, const void* key, void** value_ptr)
{
	if(map == NULL || key == NULL || value_ptr == NULL || *value_ptr == NULL)
        return MAP_UNINITIALIZED_ARGS;
