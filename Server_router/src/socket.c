...
static void elementDestroy (void* item)
{
    if(item != NULL)
    {
        free(item);
...
Socket* socket_create()
{
    ...
    socket->m_connected_sockets = ListCreate();
    ...
    socket->m_deleted_sockets = ListCreate();
    if(socket->m_deleted_sockets == NULL)
    {
        perror("ListCreate fail: \n");
        ListDestroy(&socket->m_connected_sockets, elementDestroy);
        free(socket->m_active_fd);
        free(socket);
        return NULL;
    }
...
void socket_destroy(Socket* socket)
{
    if(socket != NULL)
    {
        ListDestroy(&socket->m_deleted_sockets, elementDestroy);
        ListDestroy(&socket->m_connected_sockets, elementDestroy);
        free(socket->m_active_fd);
...
void delete_less_active_client(Socket* socket)
{
    ...
    ListItr it = ListItrEnd(socket->m_connected_sockets);
    it = ListItrPrev(it);
...
static void push_to_list(List* list, int client_socket)
{
    ...
    *client_socket_ptr = client_socket;
    ListPushHead(list, (void*)(client_socket_ptr));
}
